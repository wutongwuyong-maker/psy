...
async def upload_test_data(
    ...
    try:
        ...
        logger.info(f"成功接收并存储用户 {test_data.user_id} 的检测数据。")
...
async def download_report(
    ...
    try:
        ...
        # 根据格式生成对应文件
        if format == "pdf":
            filepath = generate_pdf_report(content, student_id)
        elif format == "excel":
...
async def upload_test_data_from_client(
    ...
    try:
        ...
        pdf_filename = f"{safe_name}_{test_data_obj.user_id}.pdf"
        ...
        logger.info(f"成功接收并存储客户端检测数据，用户ID: {test_data_obj.user_id}")
        return db_test_record
